 ...
 */
@Component
public class PayCurrencyISOCheckRule implements BusinessRule {

    @Override
    public String validate(Trade trade) {
        if(trade!=null && trade.getPayCcy() !=null) {
            try {
                Currency.getInstance(trade.getPayCcy());
            } catch (IllegalArgumentException e) {
                return "payCcy" + ApplicationConstant.NON_VALID_ISO_VALIDATION;
            }
